...
describe('ChatHistory', () => {
  ...
  describe('when disabled (read-only)', () => {
    it('still lets the trigger open the modal, so an existing history stays viewable', async () => {
      const page = await newSpecPage({
        components: [ChatHistory],
        html: '<chat-history disabled chat-history-value="saved history"></chat-history>',
      });

      expect(getTrigger(page).hasAttribute('disabled')).toBe(false);

      await openModal(page);

      expect(getBackdrop(page)).not.toBeNull();
      expect(getTextareaValue(getTextarea(page)!)).toBe('saved history');
    });

    it('makes the textarea read-only and hides Save, offering only Close', async () => {
      const page = await newSpecPage({
        components: [ChatHistory],
        html: '<chat-history disabled></chat-history>',
      });
      await openModal(page);

      expect(getTextarea(page)!.hasAttribute('readonly')).toBe(true);
    ...
    it('does not emit chatHistoryChange even if handleSave were somehow triggered', async () => {
      const page = await newSpecPage({
        components: [ChatHistory],
        html: '<chat-history disabled></chat-history>',
      });
      ...
      (page.rootInstance as unknown as { handleSave: () => void }).handleSave();
      await page.waitForChanges();
